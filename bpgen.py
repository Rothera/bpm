...
def process_emotes(filename, data, css_rules, js_map, seen):
...
def process_emote(name, props, selectors, css_rules, js_map, seen):
    ...
    selector = ".bpmotes-%s" % (name.lstrip("/"))
...
def main():
    ...
    parser.add_argument("--css", help="Output CSS file", type=argparse.FileType("w"), default="data/emote_classes.css")
    ...
    for file in args.yaml:
        process_emotes(file.name, yaml.load(file), css_rules, js_map, seen)
