...
def fx_package(ctx):
    ...
    if newer(glob_all(["build/firefox/*", "build/firefox/data/*", "build/firefox/lib/*"]),
        ...
        ctx.run("cfx", "xpi", "--update-url=http://rainbow.adery.net/betterponymotes.update.rdf", "--pkgdir=build/firefox", "--force-mobile")
...
def cr_package(ctx):
    ...
    with ctx.cd(dest="build/chrome"):
        ...
        ctx.copy("addon/extracss-pure.css") # Maybe someday.
...
UserscriptResourcePrefix = "http://rainbow.adery.net"
...
def gen_update_manifests(ctx):
    if newer(["build/betterponymotes.xpi"], ["build/betterponymotes.update.rdf"]):
        ...
        ctx.run("uhura", "-k", KeyFile, "build/betterponymotes.xpi",
                "http://rainbow.adery.net/betterponymotes_%s.xpi" % (ctx.vars["version"]),
    ...
    if newer(["build/betterponymotes.oex"], ["build/opera-updates.xml"]):
        ...
        open("build/opera-updates.xml", "w").write(
            ...
            'src="http://rainbow.adery.net/betterponymotes_%s.oex" version="%s"/>\n' % (version, version))
...
def sync(ctx):
    ...
    ctx.run("rsync", "-zvLr", "--delete", "www/", "ref@lyra.adery.net:~/www")
