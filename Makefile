...
build/emote-classes.css build/bpm-data.js: bpgen.py emotes/*.yaml data/rules.yaml
	./bpgen.py

build/betterponymotes.xpi: build/emote-classes.css build/bpm-data.js firefox/data/* firefox/package.json firefox/lib/main.js
	rm -f build/*.xpi
...
build/chrome.zip: build/emote-classes.css build/bpm-data.js chrome/*
	rm -f build/chrome.zip
	cd chrome && zip -r -0 -q ../build/chrome.zip * && cd ..

build/betterponymotes.oex: build/emote-classes.css build/bpm-data.js opera/includes/* opera/*
	rm -f build/*.oex
	cd opera && zip -r -q ../build/betterponymotes.oex * && cd ..
	cp build/betterponymotes.oex build/betterponymotes_`bin/version.py get`.oex

build/chrome: build/emote-classes.css build/bpm-data.js chrome/*
	cp -RL chrome build
